...
const Participants = ({ participants }: Props) => {
  ...
  const hasResult = participants.every((p) => p.score !== undefined && p.score !== "");
  //calculate the average if all participants have voted, do not count the '?' vote
  const resultVal = hasResult ? participants.reduce((acc, p) => {
    if (p.score === '?') return acc;
    return acc + parseInt(p.score || '0');
  }, 0) / participants.filter((p) => p.score !== '?').length : 0;
      ...
      <ul className="flex flex-col gap-2">
        ...
        {participants.map((participant) => (
          <li className="flex gap-4" key={participant.id}>
            ...
            {participant.score && <span className="animate-fadeIn" key={`${participant.name}_${participant.score}`}>{participant.score}</span>}
          </li>
        ))}
      </ul>
      {hasResult && <p className="font-bold bg-orange-100 text-orange-600 rounded-lg w-fit p-2">Average: {resultVal.toFixed(2)}</p>}
    </section>
  );
};
